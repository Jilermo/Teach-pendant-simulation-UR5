...
fig, ax = plt.subplots()
plt.subplots_adjust(left=0.34,bottom=0.28,right=1,top=1)
...
def ur5movec(p1,p2,sem):
    ...
    ym=p1y+dy*0.5
    zm=p1z+dz*0.5
    r=np.sqrt((p1x-xm)**2+(p1y-ym)**2+(p1z-zm)**2)
    eqesfera=(x-xm)**2+(y-ym)**2+(z-zm)**2-r**2
    print(eqesfera)
    # primer condicional a partir de aqui
    if dx==0 and dy==0:
        nvx=-dz
        nvy=0
        nvz=dx
        eqplano=nvx*(x-p1x)+nvy*(y-p1y)+nvz*(z-p1z)
        print(eqplano)
        eqcirculo=sp.nonlinsolve([eqesfera,eqplano],[x,y,z])
        print(eqcirculo.args[0])
        print(eqcirculo.args[1])
        var=eqcirculo.free_symbols
        eqcirnp1=sp.lambdify(var, eqcirculo.args[0],"numpy")
        eqcirnp2=sp.lambdify(var, eqcirculo.args[1],"numpy")
        print(eqcirnp1)
        p1ea=mrot2eangle(p1)
        angrot=obtangrot(p2,p1ea)
        angrot[0]=angrot[0]/30
        ...
        resx=resx[0]
        print(resx)
        print(np.around(eqcirnp1(p1z)[0],4))
        resx=eqcirnp1(p1z)
        if p1z>p2z:
            print("**************")
            print(p1z)
            print(p2z)
            rango2=np.arange(start=np.around(p1z,4),stop=np.around(p2z,4),step=-(p1z-p2z)/50)
            print(rango2)
            puntos2=eqcirnp1(rango2)
            print(p1)
            print(p2)
            print(puntos2)
            while n<50:
            ...
            rango2=np.arange(start=np.around(p1z,4),stop=np.around(p2z,4),step=((p2z-p1z)/50))
            puntos2=eqcirnp1(rango2)
            print(puntos2)
            while n<50:
                pn=p1@rotaz(angroti[2])@rotay(angroti[1])@rotax(angroti[0])
        ...
        # segundo condicional a partir de aqui
    else:
        ...
        print(p1y)
        print(np.around(eqcirnp1(p1z)[0],4))
        print(np.around(eqcirnp1(p1z)[1],4))
        print("******")
        print(p2x)
        print(p2y)
        print(np.around(eqcirnp2(p1z)[0],4))
        print(np.around(eqcirnp2(p1z)[1],4))
        
        if np.around(eqcirnp1(p1z)[0],4)==p1x and np.around(eqcirnp1(p1z)[1],4)==p1y:
            rango1=np.arange(start=np.around(zm+r,4),stop=np.around(p2z,4),step=-(zm+r-p2z)/50)
            puntos1=eqcirnp1(np.around(rango1,4))
            rango2=np.arange(start=np.around(p1z,4),stop=np.around(zm+r,4),step=(zm+r-p1z)/50)
            puntos2=eqcirnp2(np.around(rango2,4))
            print(puntos1)
            print(puntos2)
            print(p1)
            print(p2)
            ...
            print(r)
            rango1=np.arange(start=np.around(zm+r,4),stop=np.around(p2z,4),step=-(zm+r-p2z)/50)
            puntos1=eqcirnp2(np.around(rango1,4))
            rango2=np.arange(start=np.around(p1z,4),stop=np.around(zm+r,4),step=(zm+r-p1z)/50)
            puntos2=eqcirnp1(np.around(rango2,4))
            checar=eqcirnp1(p1z)
            print(rango1)
            print(rango2)
            print(p1)
...
def obtangrotv(t1,t2,t3,p1ea):
    ...
    #t0=rotax(p1ea[0])@rotay(p1ea[1])@rotaz(p1ea[2])
    t=t0@rotaz(t3)@rotay(t2)@rotax(t1)
    return t
...
#sem=[0,0,0,0,0,0]
#p1=mtr
#p2=mtr@trasx(100)@trasy(100)@trasz(100)
#ur5movec(p1,p2,sem)
...
axbtngrabar=plt.axes([0.42,0.06,0.15,0.06])
axbtnsecuencia=plt.axes([0.06,0.13,0.22,0.06])
axbtnreset=plt.axes([0.06,0.06,0.11,0.06])
axbtnmove=plt.axes([0.6,0.06,0.15,0.06])
axtxtpuntos=plt.axes([0.01,0.2,0.34,0.75])

btnmode=Button(axbtnmode,'modo: Joint')
button1=Button(axbtn1,'cerrar')
btngrabar=Button(axbtngrabar,'Gabar punto')
...
def grabar(event):
    ...
    if conpuntos<9:
        conpuntos+=1
        puntos[conpuntos,:,:]=mtr
        puntostetas[conpuntos,:]=tetas
        n=0
        texto=""
        if conpuntos>0:
            puntosmove[conpuntos-1]=movesel
            while n<conpuntos:
                if puntosmove[n]==1:
        ...
        print("No hay mas espacios para grabar puntos")

def mode(event):
    global modo
    if modo==1:
        modo=2
        btnmode.label.set_text("modo: Tool")
    elif modo==2:
        modo=3
        btnmode.label.set_text("modo: World")
    elif modo==3:
        modo=1
        btnmode.label.set_text("modo: Joint")
...
def realizarsecuencia(event):
    ...
    if conpuntos>0:
        while n<conpuntos:
            if puntosmove[n]==1:
                tetas1=puntostetas[n,:]
                tetas2=puntostetas[n+1,:]
                ur5movej(tetas1,tetas2)
            elif puntosmove[n]==2:
                tetas1=puntostetas[n,:]
                p1=puntos[n,:,:]
                p2=puntos[n+1,:,:]
                ur5movel(p1,p2,tetas1)
            elif puntosmove[n]==3:
                tetas1=puntostetas[n,:]
                p1=puntos[n,:,:]
                p2=puntos[n+1,:,:]
...
def resetpuntos(event):
    global conpuntos
    conpuntos=-1
    txtpuntos.set_val("Empieza a grabar puntos")
...
btnmode.on_clicked(mode)
btnmove.on_clicked(move)
btngrabar.on_clicked(grabar)
btnsecuencia.on_clicked(realizarsecuencia)
btnreset.on_clicked(resetpuntos)
